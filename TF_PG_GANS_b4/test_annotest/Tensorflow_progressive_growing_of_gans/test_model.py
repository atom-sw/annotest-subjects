...
class Test_TopLevelFunctions(unittest.TestCase):
    ...
    @hy.given(num_channels=st.just(1), resolution=st.just(32), label_size=
        ...
        fmap_max=st.just(256), latent_size=st.just(None), normalize_latents
        =st.just(True), use_wscale=st.just(True), use_pixelnorm=st.just(
        True), use_leakyrelu=st.just(True), use_batchnorm=st.just(False),
        tanh_at_end=st.just(None))
    ...
    @hy.given(num_channels=st.one_of(st.integers(min_value=1, max_value=
        ...
        1024, 2048, 4096]), st.just(256)), mbstat_func=st.one_of(st.
        sampled_from(['Tstdeps']), st.just('Tstdeps')), mbstat_avg=st.
        one_of(st.sampled_from([None, 'all', 'flat', 'spatial', 'none',
        'gpool']), st.just('all')), mbdisc_kernels=st.one_of(st.one_of(st.
        integers(min_value=0, max_value=1000), ), st.just(None)),
        use_wscale=st.one_of(st.sampled_from([True, False]), st.just(True)),
        use_gdrop=st.one_of(st.sampled_from([True, False]), st.just(True)),
        use_layernorm=st.one_of(st.sampled_from([True, False]), st.just(False))
        )
    @hy.settings(deadline=None)
