...
import os
Path_to_file = (os.path.expanduser('~') + "/annotest_subjects_data/tf_ThinPlateSpline_b22/original.png")
...
def ThinPlateSpline2(U, source, target, out_size):
  ...
  def _solve_system(source, target):
    ...
    W_1 = tf.concat(2, [  # repo_bug 1/2
      zeros, tf.transpose(p, [0, 2, 1])]) # [bn, 3, pn+3]  # repo_bug 2/2
