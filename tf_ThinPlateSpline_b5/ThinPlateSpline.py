...
import os
Path_to_file = (os.path.expanduser('~') + "/annotest_subjects_data/tf_ThinPlateSpline_b5/original.png")
...
def ThinPlateSpline(U, coord, vector, out_size):
  ...
  def _meshgrid(height, width, coord):
    ...
    grid = tf.concat(1, [ones, x_t_flat_g, y_t_flat_g, r]) # [bn, 3+pn, h*w]  # repo_bug
    return grid
