...
bug_id_data_directory_path = (os.path.expanduser('~') +  # repo_change
                              "/annotest_subjects_data/cnn_text_classification_tf_b9/")  # repo_change
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        # Train Summaries
        # train_summary_op = tf.merge_summary([loss_summary, acc_summary, grad_summaries_merged])  # repo_change
        train_summary_op = tf.summary.merge([loss_summary, acc_summary, grad_summaries_merged])  # repo_change
        train_summary_dir = os.path.join(out_dir, "summaries", "train")
        # train_summary_writer = tf.train.SummaryWriter(train_summary_dir, sess.graph)  # repo_change
        train_summary_writer = tf.summary.FileWriter(train_summary_dir, sess.graph)  # repo_change

        # Dev summaries
        # dev_summary_op = tf.merge_summary([loss_summary, acc_summary])  # repo_change
        dev_summary_op = tf.summary.merge([loss_summary, acc_summary])  # repo_change
        dev_summary_dir = os.path.join(out_dir, "summaries", "dev")
        dev_summary_writer = tf.train.SummaryWriter(dev_summary_dir, sess.graph)  # repo_bug
