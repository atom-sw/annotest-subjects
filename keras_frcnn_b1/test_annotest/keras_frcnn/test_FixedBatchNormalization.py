...
class test_ImportModule(unittest.TestCase):
    ...
    def test_import_module(self):
        import keras_frcnn.FixedBatchNormalization
        x = keras_frcnn.FixedBatchNormalization


class Test_FixedBatchNormalization(unittest.TestCase):

    @hy.given(epsilon=st.just(0.001), axis=st.just(-1), weights=st.just(
        None), beta_init=st.just('zero'), gamma_init=st.just('one'),
        gamma_regularizer=st.just(None), beta_regularizer=st.just(None))
    ...
    def test_get_config(self, st_for_data):
        ...
        cc_axis = st_for_data.draw(st.just(-1))
