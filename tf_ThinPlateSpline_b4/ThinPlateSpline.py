...
import os
Path_to_file = (os.path.expanduser('~') + "/annotest_subjects_data/tf_ThinPlateSpline_b4/original.png")
...
def ThinPlateSpline(U, coord, vector, out_size):
  ...
  def _meshgrid(height, width, coord):
    ...
    y_t_flat_g = tf.tile(y_t_flat, tf.pack([num_batch, 1, 1])) # [bn, 1, h*w]  # repo_bug
