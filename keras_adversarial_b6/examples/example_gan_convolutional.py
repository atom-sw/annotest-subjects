...
def model_discriminator(input_shape=(1, 28, 28), dropout_rate=0.5):
    ...
    nch = 512
    # nch = 128
    # H = Convolution2D(nch / 2, 5, 5, subsample=(2, 2), border_mode='same', activation='relu')(d_input)  # repo_change
    H = Convolution2D(int(nch / 2), 5, 5, subsample=(2, 2), border_mode='same', activation='relu')(d_input)  # repo_change
    H = LeakyReLU(0.2)(H)
