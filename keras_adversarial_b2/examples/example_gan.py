...
def model_discriminator(input_shape, hidden_dim=1024, reg=lambda: l1l2(1e-5, 1e-5), output_activation="sigmoid"):
    return Sequential([
        ...
        LeakyReLU(0.2),
        # Dense(hidden_dim / 2, name="discriminator_h2", W_regularizer=reg()),  # repo_change
        Dense(int(hidden_dim / 2), name="discriminator_h2", W_regularizer=reg()),  # repo_change
        LeakyReLU(0.2),
        Dense(hidden_dim / 4, name="discriminator_h3", W_regularizer=reg()),  # repo_bug
        LeakyReLU(0.2),
