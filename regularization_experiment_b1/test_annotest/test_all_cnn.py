...
from all_cnn import main, parse_arg
...
class Test_TopLevelFunctions(unittest.TestCase):
    ...
    @hy.given(nb_epoch=st.just(50), data_augmentation=st.just(F), noise=st.
        just(True), maxout=st.just(True), dropout=st.just(True), l1_reg=st.
        just(False), l2_reg=st.just(True), max_pooling=st.just(True), deep=
        st.just(False), noise_sigma=st.just(0.01))
    @hy.settings(deadline=None)
